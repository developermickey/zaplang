...
export function activate(context: vscode.ExtensionContext) {
  ...
  const runFile = vscode.commands.registerCommand("zap.runFile", () => {
    ...
    if (!editor) return showError("No active Zap file open")
  ...
  if (vscode.window.activeTextEditor) {
    ...
    if (fn.endsWith(".zap") || fn.endsWith(".zp")) statusBar.show()
  }
...
function getZapPath(): string {
  return vscode.workspace.getConfiguration("zap").get<string>("zapPath") ?? "zap"
}

let _terminal: vscode.Terminal | null = null
function getOrCreateTerminal(): vscode.Terminal {
  if (_terminal && !(_terminal as any).exitStatus) return _terminal
  _terminal = vscode.window.createTerminal("⚡ Zap")
  return _terminal
}

// ── Hover docs for built-in functions ────────────────────────────────
const HOVER_DOCS: Record<string, { signature: string; doc: string; example?: string }> = {
